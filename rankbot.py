...
def checkcall():
    ...
    if(data["result"]["comm_data"]):
        ...
        diff_time_start = (datetime.now(timezone('Asia/Tokyo')) - (parser.parse(data2["result"]["comm_data"]["event_start"].replace('2099',datetime.now(timezone('Asia/Tokyo')).year)))).total_seconds()
